...
public class Server {
	...
	// Queue of tags to search (for example from different users)
	...
	public static void main(String[] args) throws Exception {		// put for test "1" in command line arguments
		...
		File dbDir = new File ("H:\\Repositories\\GitHub\\Projects\\InstagramSearchEngine\\data\\DB");					// directory with all 3 DBs (used for search by tag)
			...
			System.out.println(Arrays.toString(list_rawDataDir));
			System.out.println();
			System.out.println(Arrays.toString(list_dbDir));
...
*/
		...
		/**
		 * After Server gets a request (tag) from a user, it creates and executes new TasksBoard object for the same Master.
		 * 
		 */
